...
describe Renshi::Parser do
  ...
  it "should interpret vars surrounded by whitespace" do 
    ...
    out = Renshi::Parser.parse(read_file("data/white_space.ren"), binding)
    doc = N(out)
  ...
  it "should evaluate r:ifs" do
    foo = true
    out = Renshi::Parser.parse(read_file("data/if_1.ren"), binding)
    doc = N(out)
  ...
  it "another r:if test" do
    foo = true
    out = Renshi::Parser.parse(read_file("data/if_2.ren"), binding)
    doc = N(out)
    (doc/"div[@id='content']").text.strip.should eql "hello"
  end
end